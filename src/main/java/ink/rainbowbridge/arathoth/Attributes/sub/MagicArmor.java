...
/**
 * @author 寒雨
 * @create 2020/12/6 10:08
 */
public class MagicArmor implements SubAttribute {
    ...
    public void register(Plugin plugin, FileConfiguration config, boolean first) {
        file = config;
        if (first) {
            config.set(getName()+".Priority",270);
    ...
    public void Action(Event e) {
        if (e instanceof EntityDamageEvent) {
            ...
            if (eve.getCause() == EntityDamageEvent.DamageCause.MAGIC) {
                ...
                eve.setDamage(EntityDamageEvent.DamageModifier.MAGIC,eve.getDamage(EntityDamageEvent.DamageModifier.MAGIC) - value);
            }
        }
    }
}
