...
/**
 * @author 寒雨
 * @create 2020/12/6 9:37
 */
public class MagicDamage implements SubAttribute {
    ...
    public void register(Plugin plugin, FileConfiguration config, boolean first) {
        file = config;
        if (first) {
            config.set(getName()+".Priority",230);
        ...
        if (e instanceof EntityDamageByEntityEvent) {
            ...
            if (event.getEntity() instanceof Player) {
                    if (event.getDamager() instanceof LivingEntity) {
                        Double damage1 = AttributeManager.getAttrData((LivingEntity) event.getDamager(), "PhysicalDamage")[0] + AttributeManager.getAttrData((LivingEntity) event.getDamager(), getName())[1] * AttributeManager.getAttrData((LivingEntity) event.getDamager(), getName())[2] / 100;
                        Double damage2 = AttributeManager.getAttrData((LivingEntity) event.getDamager(), "PhysicalDamage")[0] + AttributeManager.getAttrData((LivingEntity) event.getDamager(), getName())[1] * AttributeManager.getAttrData((LivingEntity) event.getDamager(), getName())[2] / 100;
                        Double damage3 = damage1 + Arathoth.random.nextDouble() * (damage2 - damage1);
                        event.setDamage(EntityDamageEvent.DamageModifier.MAGIC,Math.floor(event.getDamage(EntityDamageEvent.DamageModifier.MAGIC) + damage3));
