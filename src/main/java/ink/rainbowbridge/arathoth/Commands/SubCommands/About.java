...
public class About implements SubCommandExecutor {
    ...
    public boolean command(CommandSender sender, String[] args) {
        if (sender instanceof Player) {
            ...
            SendUtils.send((Player) sender, "Version: &f"+Arathoth.getInstance().getDescription().getVersion());
        } else {
            SendUtils.info("&f&lArathoth &7- &8&lZero");
            SendUtils.info("Version: &f"+Arathoth.getInstance().getDescription().getVersion());
        }
        return true;
    }
}